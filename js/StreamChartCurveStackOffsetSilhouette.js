export default function StreamChartCurveStackOffsetSilhouette(
  aqTable,
  canvas,
  simulation
) {
  ...
  function chart() {
    ...
    const stack = d3.stack().keys(keyArray).offset(d3.stackOffsetSilhouette);
    ...
    path.join(
      ...
      function (exit) {
        const rectExitTransition = exit.transition().attr("opacity", 0);
